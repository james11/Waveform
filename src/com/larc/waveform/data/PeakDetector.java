...
public class PeakDetector {
	...
	private static final int DETECTOR_SIZE = 31;
	...
	public boolean checkPeak(){
		...
		mIndex = index;
		if (hasPeak && (max - mBase)>mAmplitude*0.7){
			mPeakTimeList.add(mTime[mIndex]);
			updateRange(max-min);
			Log.v(TAG , "beat: amplitude=" + (max - mBase)+" base="+mBase+" diff="+(max-min));
			return true;
		} else {
			updateBase(min);
			return false;
	...
	private void updateBase(float newValue){
		...
		mBase = newValue * BASE_UPDATE_FACTOR + (1-BASE_UPDATE_FACTOR) * mBase;
	}
	
	private void updateRange(float newValue){
		
		mAmplitude = newValue * AMP_UPDATE_FACTOR + (1-AMP_UPDATE_FACTOR) * mAmplitude;
	}
	...
	public void putData(byte[] data, int offset, int length) {
		for (int i=offset; i< length; i+=10){
			putData(data[i] & 0xFF);
		}
	}
}
